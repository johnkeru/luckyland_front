...
import WasteTRCell from './WasteTRCell'

const WasteBody = ({ loading, configMethods, data, isAllow }) => {
                ...
                <TableBody>
                    {
                        data.data.length === 0 ? <TableRow>
                            ...
                            data.data.map((row, index) =>
                                <WasteTRCell isAllow={isAllow} configMethods={configMethods} row={row} index={index} key={index} />)
                    }
                    {emptyRows > 0 && (
                        <TableRow
                            style={{
